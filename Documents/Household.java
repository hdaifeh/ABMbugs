 ...
 * model. It tracks waste production and behavioral intentions related to waste
 * management practices like composting and sorting. Modified to handle dynamic
...
public class Household {
    ...
    public void computeBehavioralIntentions(int y) {
        ...
        αcf = Math.min(territory.αcf_initial + (1 - territory.αcf_initial) * territory.sigmoide_mcf[Math.max(0, y - 1)], 1.0);
        αcg = Math.min(territory.αcg_initial + (1 - territory.αcg_initial) * territory.sigmoide_mcg[Math.max(0, y - 1)], 1.0);
        αsf = territory.αsf_initial + (1 - territory.αsf_initial) * territory.sigmoide_msf[y];
        αsg = territory.αsg_initial + (territory.αsg_max - territory.αsg_initial) * territory.sigmoide_msg[y];

        // Ensure constraints are respected
        if ((trucDa = αcf + αsf) > 1.0) {
            αsf = 1 - αcf;
        }
        αsg = αsg_initial + (αsg_max - αsg_initial) * territory.sigmoide_msg[y];

        if ((trucDv = αcg + αsg) > 1.0) {
            αsg = 1 - αcg;
        }
        αvg = 1 - αcg - αsg;
    }
    ...
    public void putInBlackBin(int y) {
        ...
        if (Br < 0) {
            System.err.println("Negative Br detected.");
        }
    }

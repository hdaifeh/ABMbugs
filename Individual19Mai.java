...
public class Individual {
    ...
    public Individual(int sizeData, double bpf, double bpg, CollectionTerritory territory) {
        ...
        this.b_pf = bpf;
        this.b_pg = bpg;
               // Base green waste production coefficient
...
public void computeWaste(int year, double αpf_target, double[] sigmoideABP) {
    // Apply reduction factor to BASE values, not cumulative values
    Bpf = b_pf * (1 - αpf_target * sigmoideABP[year]);
    Bpg = b_pg * (1 - αpf_target * sigmoideABP[year]);
}

...
/**
 * Modified to read equipment capacity file
 */
public void lectureEntree(String fileName, int ligne, int frequenceSauvegarde, boolean entete) {
    try {
        ...
        for (int a = 0; a < nbSubterritories; a++) {
            ...
            st.nextToken();
            paramSubT[15] = 0.0; // Set Kc_init to 0
            st.nextToken();
            paramSubT[16] = (double) st.nval;
            st.nextToken();
            paramSubT[17] = 0.0; // Set Ks_init to 0
            st.nextToken();
        ...
        for (int i = 0; i < nbSubterritories; i++) {
            myDyn.myTerrit[i].readEquipmentCapacityFile("equipment_capacity.txt");
        }
